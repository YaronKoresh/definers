...
class TestExtractVideoFeatures(unittest.TestCase):
    ...
    def test_custom_frame_interval(self):
        frame_interval = 5
        features = extract_video_features(self.video_path, frame_interval=frame_interval)
        self.assertIsNotNone(features)
    ...
    def test_empty_video(self):
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(empty_video_path, fourcc, self.fps, (self.width, self.height))
        out.release()
