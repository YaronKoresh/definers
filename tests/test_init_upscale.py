...
import torch
from definers import init_upscale, MODELS
...
class TestInitUpscale(unittest.TestCase):
    ...
    @patch('torch.load')
    def test_successful_initialization(self, mock_torch_load, mock_hf_hub_download, mock_dtype, mock_device, mock_pillow_heif):
    ...
    @patch('definers.hf_hub_download', return_value='mock/path/model.pth')
    @patch('torch.load')
    def test_model_assigned_correctly(self, mock_torch_load, mock_hf_hub_download, mock_dtype, mock_device, mock_pillow_heif):
