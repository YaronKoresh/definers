...
def test_run_command_returns_stdout_lines_and_merges_env(monkeypatch: pytest.MonkeyPatch) -> None:
    logged: list[tuple[str, str]] = []
    monkeypatch.setitem(
        sys.modules,
        "definers.file_ops",
        SimpleNamespace(log=lambda message, detail: logged.append((message, detail)), catch=lambda error: None),
    )
    ...
    result = processes._run_command(["python", "-V"], silent=True, env={"LANE": "4"})
...
def test_run_command_rejects_unsafe_command_and_reports_catch(monkeypatch: pytest.MonkeyPatch) -> None:
    caught: list[Exception] = []
    monkeypatch.setitem(
        sys.modules,
        "definers.file_ops",
        SimpleNamespace(log=lambda message, detail: None, catch=lambda error: caught.append(error)),
    )
    popen = MagicMock()
...
def test_run_command_returns_false_and_logs_on_non_zero_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    logged: list[tuple[str, str]] = []
    monkeypatch.setitem(
        sys.modules,
        "definers.file_ops",
...
def test_run_command_catches_popen_exceptions(monkeypatch: pytest.MonkeyPatch) -> None:
    caught: list[Exception] = []
    monkeypatch.setitem(
        sys.modules,
        "definers.file_ops",
...
def test_run_linux_delegates_to_run_command(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[tuple[object, bool, object]] = []
    monkeypatch.setattr(
        processes,
        "_run_command",
        lambda command, silent=False, env=None: calls.append((command, silent, env)) or ["ok"],
    )
...
def test_run_windows_delegates_to_run_command(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[tuple[object, bool, object]] = []
    monkeypatch.setattr(
        processes,
        "_run_command",
...
def test_get_process_pid_returns_parsed_pid(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(processes.subprocess, "check_output", lambda args: b"12345\n")

    assert processes.get_process_pid("python") == 12345


def test_get_process_pid_returns_none_for_missing_process(monkeypatch: pytest.MonkeyPatch) -> None:
    def raise_called_process_error(args: object) -> bytes:
        raise subprocess.CalledProcessError(1, args)

    monkeypatch.setattr(processes.subprocess, "check_output", raise_called_process_error)

    assert processes.get_process_pid("missing") is None


def test_send_signal_to_process_returns_true_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    kill_calls: list[tuple[int, int]] = []
    monkeypatch.setattr(processes.os, "kill", lambda pid, signal_number: kill_calls.append((pid, signal_number)))

    assert processes.send_signal_to_process(100, 15) is True
    assert kill_calls == [(100, 15)]


def test_send_signal_to_process_returns_false_on_os_error(monkeypatch: pytest.MonkeyPatch) -> None:
