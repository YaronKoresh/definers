...
class TestGetMaxResolution(unittest.TestCase):
    ...
    def test_basic_calculation_16_9(self):
        ...
        self.assertLessEqual(
            new_width * new_height, 0.25 * 1000 * 1000
        )
        self.assertTrue(new_width % 16 == 0)
        self.assertTrue(new_height % 16 == 0)
        self.assertAlmostEqual(
            width / height, new_width / new_height, places=2
        )
    ...
    def test_custom_megapixels(self):
        ...
        new_width, new_height = get_max_resolution(
            width, height, mega_pixels=1.0
        )
        self.assertLessEqual(
            new_width * new_height, 1.0 * 1000 * 1000
        )
        self.assertAlmostEqual(
            width / height, new_width / new_height, places=2
        )
    ...
    def test_large_input_downscaling(self):
        ...
        new_width, new_height = get_max_resolution(
            width, height, mega_pixels=0.5
        )
        self.assertLessEqual(
            new_width * new_height, 0.5 * 1000 * 1000
        )
