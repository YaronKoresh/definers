...
class TestPredictAudio(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher_timeline = patch('definers.get_active_audio_timeline', return_value=[(0.5, 1.5)])
        self.patcher_features_to_audio = patch('definers.features_to_audio', return_value=np.random.randn(self.sr))
        self.patcher_tmp = patch('definers.tmp', return_value=os.path.join(self.test_dir, "output.wav"))
        self.patcher_is_clusters = patch('definers.is_clusters_model', return_value=False)
        self.patcher_get_cluster = patch('definers.get_cluster_content', return_value=np.random.rand(100))
