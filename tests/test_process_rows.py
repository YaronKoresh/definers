...
from definers import process_rows, cupy_to_numpy
...
class TestProcessRows(unittest.TestCase):
    ...
    def _run_process_rows_test(self, mock_scaler_cls, mock_normalizer_cls, mock_imputer_cls):
    ...
    def test_process_rows_with_cuml(self, mock_scaler_cuml, mock_normalizer_cuml, mock_imputer_cuml):
        self._run_process_rows_test(mock_scaler_cuml, mock_normalizer_cuml, mock_imputer_cuml)

    @patch('definers.SimpleImputer', create=True)
    @patch('definers.Normalizer', create=True)
    @patch('definers.StandardScaler', create=True)
    def test_process_rows_with_sklearn_fallback(self, mock_scaler_sklearn, mock_normalizer_sklearn, mock_imputer_sklearn):
        with patch.dict('sys.modules', {'cuml': None}):
             self._run_process_rows_test(mock_scaler_sklearn, mock_normalizer_sklearn, mock_imputer_sklearn)
    ...
    @patch('definers.Normalizer')
    @patch('definers.StandardScaler')
    def test_single_row_input(self, mock_scaler_cls, mock_normalizer_cls, mock_imputer_cls):
        mock_scaler_cls.return_value = self.mock_scaler
        mock_normalizer_cls.return_value = self.mock_normalizer
        mock_imputer_cls.return_value = self.mock_imputer
