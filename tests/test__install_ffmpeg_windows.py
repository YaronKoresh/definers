...
class TestInstallFfmpegWindows(unittest.TestCase):
    ...
    def test_not_admin_exits(self, mock_print, mock_exit, mock_is_admin):
        _install_ffmpeg_windows()
        mock_exit.assert_called_once_with(1)
        mock_print.assert_any_call("[ERROR] This script requires Administrator privileges to run on Windows.")

    @patch('definers.is_admin_windows', return_value=True)
    @patch('subprocess.run')
    @patch('builtins.print')
    def test_winget_success(self, mock_print, mock_run, mock_is_admin):
        ...
        mock_print.assert_any_call("[SUCCESS] FFmpeg has been installed via Winget.")
    ...
    def test_manual_install_success_after_winget_fail(
        self, mock_exit, mock_print, mock_gettempdir, mock_remove, mock_rmtree,
        mock_move, mock_listdir, mock_exists, mock_zipfile, mock_requests_get,
        mock_subprocess_run, mock_is_admin
    ):
        ...
        with zipfile.ZipFile(zip_content, 'w') as zf:
            zf.writestr('ffmpeg-build-dir/bin/ffmpeg.exe', b'ffmpeg_data')
        zip_content.seek(0)
        ...
        mock_zip_instance.extractall.assert_called_with('/tmp/ffmpeg_extracted')
        
        program_files = os.environ.get("ProgramFiles", "C:\\Program Files")
        ffmpeg_install_dir = os.path.join(program_files, "ffmpeg")

        mock_move.assert_called_once_with(
            os.path.join('/tmp/ffmpeg_extracted', 'ffmpeg-build-dir', 'bin'),
            ffmpeg_install_dir
        )
        ...
        mock_print.assert_any_call("[SUCCESS] FFmpeg added to system PATH.")
    ...
    def test_manual_install_download_fails(
        self, mock_exit, mock_print, mock_requests_get,
        mock_subprocess_run, mock_is_admin
    ):
        _install_ffmpeg_windows()
