...
class TestFilesToDataset(unittest.TestCase):
    ...
    def test_files_to_dataset_with_features_and_labels(
        ...
        mock_load_as_numpy.side_effect = [
            np.array([1]),
            np.array([2]),
        ]
    ...
    def test_files_to_dataset_with_features_only(
        ...
        mock_cupy_to_numpy.side_effect = lambda x: x
        mock_tensor_dataset.return_value = (
            "FeaturesOnly_TensorDataset_Object"
        )
        ...
        mock_load_as_numpy.assert_called_once_with(
            "feature.npy", training=True
        )
        mock_tensor_dataset.assert_called_once()
    ...
    def test_files_to_dataset_loading_feature_fails(
        self, mock_print, mock_load_as_numpy
    ):
    ...
    @patch("definers.load_as_numpy")
    @patch("builtins.print")
    def test_files_to_dataset_loading_label_fails(
        self, mock_print, mock_load_as_numpy
    ):
    ...
    @patch("definers.load_as_numpy")
    @patch("torch.utils.data.TensorDataset")
    def test_files_to_dataset_handles_list_from_loader(
        self, mock_tensor_dataset, mock_load_as_numpy
    ):
        mock_load_as_numpy.return_value = [
            np.array([1]),
            np.array([2]),
        ]
        ...
        self.assertEqual(
            len(mock_tensor_dataset.call_args[0][0][0]), 2
        )
    ...
    @patch(
        "torch.stack", side_effect=Exception("Tensor stacking failed")
    )
    def test_files_to_dataset_tensor_creation_fails(
        self, mock_stack, mock_catch, mock_load
    ):
        ...
        self.assertIn(
            "Tensor stacking failed", str(mock_catch.call_args[0][0])
        )
