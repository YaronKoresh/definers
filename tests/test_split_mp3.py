...
class TestSplitMp3(unittest.TestCase):
    ...
    @patch('definers.Path')
    def test_split_audio_evenly(self, mock_path_cls, mock_audio_segment_cls):
        ...
        self.assertTrue(export_path.startswith(f'{os.getcwd()}/mp3_segments_'))
        self.assertEqual(num_chunks, 6)
        
        mock_path_instance.mkdir.assert_called_once_with(parents=True, exist_ok=True)
        mock_audio_segment_cls.from_mp3.assert_called_with('dummy.mp3')
    ...
    @patch('definers.AudioSegment')
    @patch('definers.Path')
    def test_split_audio_unevenly(self, mock_path_cls, mock_audio_segment_cls):
    ...
    @patch('definers.AudioSegment')
    @patch('definers.Path')
    def test_audio_shorter_than_chunk(self, mock_path_cls, mock_audio_segment_cls):
