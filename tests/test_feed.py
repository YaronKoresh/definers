...
class TestFeed(unittest.TestCase):
    ...
    def test_feed_unsupervised_initial(self, mock_one_dim_numpy, mock_log):
        ...
        mock_log.assert_called_once_with("Feeding epoch 1 X", self.X_new_np)

    @patch('definers.log')
    @patch('definers.one_dim_numpy', side_effect=lambda x: x)
    @patch('definers.np.concatenate', side_effect=lambda arrays, axis: np.concatenate(arrays, axis=axis))
    def test_feed_unsupervised_append(self, mock_concatenate, mock_one_dim_numpy, mock_log):
    ...
    @patch('definers.one_dim_numpy', side_effect=lambda x: x)
    @patch('definers.np.concatenate', side_effect=lambda arrays, axis: np.concatenate(arrays, axis=axis))
    def test_feed_supervised_append(self, mock_concatenate, mock_one_dim_numpy, mock_log):
        initial_model = HybridModel()
        initial_model.X_all = np.array([[0, 0]])
