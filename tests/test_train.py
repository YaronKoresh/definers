...
class TestTrain(unittest.TestCase):
    ...
    @patch("joblib.load")
    @patch("joblib.dump")
    def test_train_with_preloaded_model(
        self,
        mock_dump,
        mock_load,
        mock_init_tokenizer,
    ...
    @patch("joblib.dump")
    def test_train_supervised_remote(
        self,
        mock_dump,
        mock_hybrid_model,
        mock_init_tokenizer,
        ...
        mock_fetch_dataset.return_value = [
            {"feature_1": "text 1", "label_1": "target 1"}
        ]
    ...
    @patch("definers.HybridModel")
    @patch("joblib.dump")
    def test_train_unsupervised_local(
        self,
        mock_dump,
        mock_hybrid_model,
        mock_init_tokenizer,
    ...
    @patch("definers.init_tokenizer")
    @patch("joblib.dump")
    def test_train_save_error(
        self, mock_dump, mock_init_tokenizer, mock_fetch_dataset, mock_sanitize
    ):
        mock_init_tokenizer.return_value = self.mock_tokenizer
        mock_fetch_dataset.return_value = [{"feature": "data"}]
