...
class TestExist(unittest.TestCase):
    ...
    def test_existing_file(self, mock_run):
        ...
        mock_run.assert_called_with('ls -1 "/path/to/existing_file.txt"', silent=True)
    ...
    def test_existing_directory(self, mock_run):
        ...
        mock_run.assert_called_with('ls -1 "/path/to/existing_dir"', silent=True)

    @patch('definers.run')
    def test_non_existing_path(self, mock_run):
        mock_run.return_value = False
        self.assertFalse(exist('/path/to/non_existing_file.txt'))
        mock_run.assert_called_with('ls -1 "/path/to/non_existing_file.txt"', silent=True)
    ...
    def test_path_with_spaces(self, mock_run):
        ...
        mock_run.assert_called_with('ls -1 "/path/to/a file with spaces.txt"', silent=True)
    ...
    @patch('definers.run')
    def test_path_expansion(self, mock_run, mock_expanduser, mock_abspath):
