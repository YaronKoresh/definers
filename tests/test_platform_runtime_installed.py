...
def test_installed_returns_true_from_windows_registry(
    ...
    monkeypatch.setattr(
        ...
        lambda command, **kwargs: (
            ...
            or SimpleNamespace(stdout="Example Tool  1.2.3\n")
        ),
    )
...
def test_installed_falls_back_to_pip_after_windows_registry_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[object] = []
    monkeypatch.setattr(runtime, "get_os_name", lambda: "windows")
...
def test_installed_returns_true_for_linux_binary_version(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[object] = []
...
def test_installed_uses_linux_v_fallback_when_version_output_is_empty(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[object] = []
    monkeypatch.setattr(runtime, "get_os_name", lambda: "linux")
...
def test_installed_returns_true_from_pip_list(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_installed_returns_false_when_nothing_matches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(runtime, "get_os_name", lambda: "linux")
