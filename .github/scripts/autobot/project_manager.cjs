...
function isExactVersionTag(rawVersion) {
  return /^v?\d+\.\d+\.\d+$/.test(String(rawVersion || "").trim());
}
...
function inferIssueLabels(issue) {
  const analysis = analyzeIssueIntake(issue);
  return AutobotLabelRegistry.technicalLabelsOnly(uniqueValidLabels([
    ...(analysis.labels || []),
...
async function getReleaseVersionTags({ github, owner, repo }) {
  try {
    const releases = await github.paginate(github.rest.repos.listReleases, { owner, repo, per_page: 100 });
    return releases
      .map((release) => String(release.tag_name || "").trim())
      .filter((tag) => isExactVersionTag(tag));
  } catch (error) {
    return [];
  }
}

function getHighestVersionTag(tags) {
  if (!Array.isArray(tags) || tags.length === 0) return null;
  return tags.reduce((maxTag, currentTag) => compareVersions(currentTag, maxTag) > 0 ? currentTag : maxTag, tags[0]);
}

function getHighestVersionTagBelow(tags, ceilingVersion) {
  return getHighestVersionTag((tags || []).filter((tag) => compareVersions(tag, ceilingVersion) < 0));
}
...
async function ensureMilestoneBaseVersion({ github, owner, repo, milestone }) {
  ...
  const releaseTags = await getReleaseVersionTags({ github, owner, repo });
  const priorReleaseVersion = getHighestVersionTagBelow(releaseTags, milestone.title);
  const baseVersion = metadata.baseVersion
    ? metadata.baseVersion
    : (priorReleaseVersion ? formatVersionTag(parseVersionTag(priorReleaseVersion)) : "v0.0.0");
...
function getInvalidLabelNames(error) {
  if (error?.status !== 422) {
    return [];
  }
  const errors = Array.isArray(error?.response?.data?.errors) ? error.response.data.errors : [];
  return [...new Set(errors
    .filter((entry) => {
      ...
      const field = String(entry?.field || "").toLowerCase();
      const code = String(entry?.code || "").toLowerCase();
      return resource === "label" && field === "name" && code === "invalid";
    })
    .map((entry) => normalizeLabelName(entry?.value || ""))
    .filter(Boolean))];
}
...
async function addLabelToIssue({ github, owner, repo, issueNumber, label }) {
  ...
  try {
    ...
    if (!isInvalidLabelNameError(error, label) && getInvalidLabelNames(error).length === 0) throw error;
    for (const invalidLabel of getInvalidLabelNames(error)) {
      await ensureLabelExists({ github, owner, repo, name: invalidLabel });
    }
    await ensureLabelExists({ github, owner, repo, name: label });
    await github.rest.issues.addLabels({ owner, repo, issue_number: issueNumber, labels: [label] });
  }
...
async function getOrCreateMilestone({ github, owner, repo }) {
  ...
  const releaseTags = await getReleaseVersionTags({ github, owner, repo });
  const latestTrackedVersion = getHighestVersionTag(releaseTags);
  const nextVersion = latestTrackedVersion ? computeTargetVersion(latestTrackedVersion, "patch") : "v0.0.1";
...
async function prepareProjectState(input) {
  ...
  if (!isPR) {
    const inferredIssueLabels = inferIssueLabels(payloadIssue);
    const nextIssueLabels = parsedAutobotLabels.length > 0
      ? uniqueValidLabels([...existingIssueLabels, ...parsedAutobotLabels, ...inferredIssueLabels])
      : uniqueValidLabels([...existingIssueLabels, ...inferredIssueLabels]);
    issueLabelsToAdd = nextIssueLabels.filter((label) => !existingIssueLabels.includes(label));
  }
...
async function syncPreparedProjectState({ github, owner, repo, issueNumber, stateFile }) {
  ...
  if (!isSyncableProjectAction(state.action)) {
    return state;
  }
...
async function syncProjectMilestone({ github, owner, repo, context, issueNumber }) {
  const payload = context.payload;
  if (!isSyncableProjectAction(payload.action)) {
    return;
  }
