...
def _load_model(
    ...
    *,
    ...
    from definers.ml.safe_deserialization import load_serialized_model
...
def _resolve_huggingface_model_source(
    value: str,
    requested_model_type: str | None,
):
    ...
    if _is_transformers_text_generation_repo(repo_files, selected_files):
        ...
        return ResolvedModelSource(
            ...
            trusted_directories=_trusted_directories_for_paths((snapshot_dir,)),
...
def _resolve_generic_remote_source(
    url: str,
    requested_model_type: str | None,
):
    ...
    if len(remote_urls) == 1:
        ...
        _validate_downloaded_model_file(downloaded_path, resolved_model_type)
        return ResolvedModelSource(
...
def _parse_huggingface_reference(value: str):
...
def _is_huggingface_reference(value: str) -> bool:
    from urllib.parse import urlparse

    normalized_value = str(value).strip()
    if _is_http_url(normalized_value):
        return urlparse(normalized_value).netloc.lower() in HUGGINGFACE_HOSTS
...
def _is_huggingface_repo_id(value: str) -> bool:
    ...
    return all(character in allowed_characters for character in user) and all(
        character in allowed_characters for character in name
    )


def _select_huggingface_files(
    reference,
    repo_files,
    requested_file,
    requested_model_type,
...
def _match_requested_repo_file(
    reference,
    repo_files,
    requested_file,
    requested_model_type,
...
def _supported_repo_files(repo_files, requested_model_type):
    supported_files = []
    for repo_file in repo_files:
        try:
...
def _expand_indexed_repo_file(reference, repo_files, index_file):
    try:
    ...
    if shard_files:
        available_files = [
            shard_file for shard_file in shard_files if shard_file in repo_files
        ]
        if available_files:
    ...
    if shard_group:
        return shard_group
    raise FileNotFoundError(f"No shards found for index file '{index_file}'.")


def _read_index_shard_files(reference, index_file):
    import json
...
def _find_repo_shard_group(repo_files, requested_file):
...
def _largest_repo_shard_group(repo_files):
...
def _group_repo_shards(repo_files):
    from pathlib import Path
    ...
    for indexed_files in grouped_files.values():
        ...
        ordered_files = tuple(
            repo_file
            for _, repo_file in sorted(indexed_files, key=lambda item: item[0])
        )
        groups.append(ordered_files)
    return groups


def _discover_remote_shard_urls(url: str):
        ...
        if total_match is not None:
            ...
            return tuple(
                _replace_remote_file_name(
                    url,
                    _format_shard_name(shard_name, index),
                )
    ...
    while upper_index - shard_name.index <= 256:
        ...
        upper_index += 1
    return tuple(url_value for _, url_value in sorted(discovered_urls.items()))
...
def _remote_file_name(url: str) -> str:
    from pathlib import Path
    from urllib.parse import urlparse
...
def _replace_remote_file_name(url: str, new_file_name: str) -> str:
    from pathlib import Path
    from urllib.parse import urlparse
...
def _parse_shard_name(file_name: str):
...
def _format_shard_name(shard_name, index: int) -> str:
    return (
...
def _resolve_model_type(value: str, requested_model_type: str | None):
    normalized_model_type = _normalize_model_type(requested_model_type)
...
def _normalize_model_type(model_type: str | None):
...
def _companion_index_path(path: str):
    from pathlib import Path

    normalized_path = str(path).strip()
    normalized_model_type = _normalize_model_type(Path(normalized_path).suffix)
    ...
    if index_suffix is None:
        return None
    return (
        f"{normalized_path}{index_suffix[len('.' + normalized_model_type) :]}"
    )


def _repo_file_matches_hint(repo_file: str, hint: str) -> bool:
    from pathlib import Path
...
def _repo_file_priority(repo_file: str):
    from pathlib import Path
...
def _is_transformers_text_generation_repo(repo_files, selected_files) -> bool:
    from pathlib import Path
...
def _should_try_auto_processor(snapshot_dir: str) -> bool:
    from pathlib import Path
...
def _load_huggingface_text_generation_model(
    snapshot_dir: str,
    *,
    task_key: str | None,
):
...
def _tokenize_generation_prompt(processor, prompt: str):
    try:
        return processor(
            text=prompt,
            images=None,
...
def _merge_mapping_parts(parts):
    merged_parts = {}
    for part in parts:
...
def _trusted_directories_for_paths(paths):
    import ntpath
    import os
    ...
    for path in paths:
        ...
        if not path_module.isabs(absolute_path):
            continue
        parent_directory = path_module.dirname(absolute_path) or absolute_path
...
def _validate_downloaded_model_file(model_path: str, model_type: str) -> None:
    from definers.ml.safe_deserialization import validate_serialized_model_file
    from definers.system import secure_path
    ...
    if lowered_header.startswith(b"version https://git-lfs.github.com/spec/v1"):
        raise ValueError(
...
def _apply_turbo_optimizations(model) -> None:
    for attr_name, args in [
        ...
        try:
            ...
            logger.warning(
                "Could not apply optimization %s: %s",
                attr_name,
                error,
            )
...
resolve_model_source = _resolve_model_source
resolve_huggingface_model_source = _resolve_huggingface_model_source
resolve_generic_remote_source = _resolve_generic_remote_source
parse_huggingface_reference = _parse_huggingface_reference
is_huggingface_reference = _is_huggingface_reference
is_huggingface_repo_id = _is_huggingface_repo_id
