...
class MlFacadeApi:
    ...
    def map_reduce_summary(text, max_words):
        from definers.ml.text_generation import map_reduce_summary

        return map_reduce_summary(text, max_words)

    @staticmethod
    def optimize_prompt_realism(prompt):
        from definers.ml.text_generation import (
    ...
    def preprocess_prompt(prompt):
        from definers.ml.text_generation import preprocess_prompt

        return preprocess_prompt(prompt)

    @staticmethod
    def summarize(text_to_summarize):
        from definers.ml.text_generation import summarize

        return summarize(text_to_summarize)

    @staticmethod
    def summary(text, max_words=20, min_loops=1):
        from definers.ml.text_generation import summary
